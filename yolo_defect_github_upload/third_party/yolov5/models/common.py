...
class SE(nn.Module):
    def __init__(self, c1, c2, r=16):  # 修正了双下划线
        super().__init__()  # 修正了双下划线
        c = max(c1 // r, 1)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Conv2d(c1, c, 1, 1, 0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(c, c1, 1, 1, 0, bias=True),  # 把 c_ 修正为了 c
            nn.Sigmoid(),
        )
...
class CBAM(nn.Module):
    def __init__(self, c1, c2, r=16, k=7):
        super().__init__()
        self.ca = ChannelAttention(c1, r)
